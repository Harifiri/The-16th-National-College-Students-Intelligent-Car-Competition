...
{
	if((tim&0x0f) > 0x05) return;																// 基础定时器不支持编码器模式
...
{
	tim_index[(tim&0x0f)]->EGR |= 0x01;															// 产生更新时间重装载计数器
}
...
// @return		void
// Sample usage:				tim_encoder_init(TIM_1, input_pin);
//-------------------------------------------------------------------------------------------------------------------
void tim_counter_init (TIM_enum tim, TIM_ENC_PIN_enum input_pin)
{
	if((tim&0x0f) > 0x05) return;																// 基础定时器不支持从模式
