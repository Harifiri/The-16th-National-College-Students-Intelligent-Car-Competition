...
ADC_TypeDef *adc_index[3] = {ADC1, ADC2, ADC3};
uint8 adc_resolution[3] = {ADC_12BIT, ADC_12BIT, ADC_12BIT};
...
// @return		void
// Sample usage:				adc_init(ADC1_CH00_A00,ADC_8BIT);//初始化A00为ADC功能 分辨率为8位
//-------------------------------------------------------------------------------------------------------------------
void adc_init (ADCN_enum adc, ADCCH_enum ch, ADCRES_enum resolution)
{
	adc_pin_init(ch);																			// 根据引脚编号来初始化引脚

	if(adc == ADC_1 && ((ch&0xF00)==0xF00))														// 如果是 ADC1 的通道15 内部参考电压
		adc_index[adc]->ADCFG |= ADC_CFGR_VEN;													// 开启内部电压传感器使能

	if(adc == ADC_1 && ((ch&0xF00)==0xE00))														// 如果是 ADC1 的通道14 内部温度传感器
		adc_index[adc]->ADCFG |= ADC_CFGR_TEN;													// 开启温度传感器使能
	...
	if((RCC->APB2ENR & (RCC_APB2ENR_ADC1 << adc)) == 0)											// 如果 ADC 未被使能
	{
		...
		adc_index[adc]->ADCR &= ~(ADC_CR_MODE | ADC_CR_ALIGN);									// 清空设置 默认单次转换 数据右对齐
...
// @param		count			均值滤波次数
// @return		uint16			转换的ADC值
// Sample usage:				adc_convert(ADC1_CH00_A00,5);//采集5次 然后返回平均值
//-------------------------------------------------------------------------------------------------------------------
