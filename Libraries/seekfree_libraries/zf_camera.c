...
// @param		pin					引脚号
// @return		void
// Sample usage:					camera_tim_etr_init(TIM_1_ETR_E07, CAMERA_GRAYSCALE);  		// 定时器ERT触发是固定的引脚禁止随意修改
//-------------------------------------------------------------------------------------------------------------------
void camera_tim_etr_init (TIM_ETR_PIN_enum pin, CAMERA_TYPE_enum camera_type)
{
	...
	switch((pin&0xF000))
	{
		...
		case 0x1000:	RCC_APB2PeriphClockCmd(RCC_APB2ENR_TIM8, ENABLE);break;
		case 0x2000:	RCC_APB1PeriphClockCmd(RCC_APB1ENR_TIM2, ENABLE);break;
		case 0x3000:	RCC_APB1PeriphClockCmd(RCC_APB1ENR_TIM5, ENABLE);break;
		case 0x4000:	RCC_APB1PeriphClockCmd(RCC_APB1ENR_TIM3, ENABLE);break;
		case 0x5000:	RCC_APB1PeriphClockCmd(RCC_APB1ENR_TIM4, ENABLE);break;
	}
	...
	if(camera_type == CAMERA_GRAYSCALE)
		TIM_ETRClockMode1Config(tim_index[((pin&0xF000)>>12)], TIM_ExtTRGPSC_OFF, TIM_ExtTRGPolarity_NonInverted, 0x00);
	else
