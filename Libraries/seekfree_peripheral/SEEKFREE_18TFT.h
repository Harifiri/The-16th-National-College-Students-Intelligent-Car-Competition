...
#define TFT_SCL			SPI3_SCK_D04												// 定义SPI_SCK引脚
#define TFT_SDA			SPI3_MOSI_D06												// 定义SPI_MOSI引脚

#define TFT_REST_PIN	D13															// 液晶复位引脚定义 由于实际通讯未使用D15因此 这里复用为GPIO
#define TFT_DC_PIN		D5															// 液晶命令位引脚定义
#define TFT_CS_PIN		D7															// 定义SPI_CS引脚
#define TFT_BL_PIN		D11															// 液晶背光引脚定义

#define TFT_DC(x)		(x? (GPIO_PIN_SET(TFT_DC_PIN)): (GPIO_PIN_RESET(TFT_DC_PIN)))
#define TFT_REST(x)		(x? (GPIO_PIN_SET(TFT_REST_PIN)): (GPIO_PIN_RESET(TFT_REST_PIN)))
#define TFT_CS(x)		(x? (GPIO_PIN_SET(TFT_CS_PIN)): (GPIO_PIN_RESET(TFT_CS_PIN)))
#define TFT_BL(x)		(x? (GPIO_PIN_SET(TFT_BL_PIN)): (GPIO_PIN_RESET(TFT_BL_PIN)))
