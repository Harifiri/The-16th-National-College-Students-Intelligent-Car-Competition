...
{
	spi_init(IPS114_SPIN, IPS114_SCL_PIN, IPS114_SDA_PIN, SPI_MISO_NULL, SPI_NSS_NULL, 0, SystemCoreClock/2);	//硬件SPI初始化
...
void mydisplay(void)
{
	...
	{
		j=(int)(25-2.78f*(19-i));
		ips114_drawpoint(j+1,i,PURPLE);
		j=(int)(75+2.78f*(19-i));
		ips114_drawpoint(j-1,i,BLUE);
	}
}
...
void ips114_displayimage032(uint8 *p, uint16 width, uint16 height) 
{
	...
	for(i=1;i<=59;i++)
	{
		ips114_drawpoint(RightEdge_X[i]-3,i,BLUE);
		ips114_drawpoint(LeftEdge_X[i]+3,i,GREEN);
		ips114_drawpoint(Middle_X[i],i,RED);
	}
}
