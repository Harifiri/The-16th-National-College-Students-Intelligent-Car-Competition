...
// @param		dat				数据
// @return		void
// Sample usage:				lcd_writeIndex(0x5A);
// @note		内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void lcd_writeIndex (uint8 dat)
{
	TFT_CS(0);																		// 软件片选控制
	TFT_DC(0);																		// DC命令数据控制引脚
...
// @param		dat				数据
// @return		void
// Sample usage:				lcd_writeData(0x5A);
// @note		内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
// @brief		LCD SPI 向液晶屏写一个16位数据
// @param		dat				数据
// @return		void
// Sample usage:				lcd_writedata_16bit(0xA55A);
// @note		内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void lcd_writedata_16bit (uint16 dat)
{
	uint8 dat1[2];
	TFT_CS(0);																		// 软件片选控制
	dat1[0] = dat >> 8;
	...
	TFT_DC(1);																		// DC命令数据控制引脚
	spi_mosi(TFT_SPIN, dat1, NULL, 2);												// 调用硬件SPI接口写入数据  高位在前  低位在后
	TFT_CS(1);																		// 软件片选控制
}
...
// @note		内部调用 用户无需关心
//-------------------------------------------------------------------------------------------------------------------
static void lcd_set_region(unsigned int x_start,unsigned int y_start,unsigned int x_end,unsigned int y_end)
{
//	assert_param(TFT_X_MAX>x_start);//参数断言，进入断言标志参数超出范围
//	assert_param(TFT_Y_MAX>y_start);

//	assert_param(TFT_X_MAX>x_end);
//	assert_param(TFT_Y_MAX>y_end);
...
void lcd_init(void)
{
	spi_init(TFT_SPIN, TFT_SCL, TFT_SDA, SPI_MISO_NULL, SPI_NSS_NULL, 0, SystemCoreClock/4);//硬件SPI初始化
